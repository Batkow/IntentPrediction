...
class Gridworld(mdp.MarkovDecisionProcess):
    ...
    def getTransitionStatesAndProbs(self, state, action):
        ...
        if action == 'northeast':
            successors.append((northEastState,1))

        if action == 'northwest':
            successors.append((northWestState,1))

        if action == 'southwest':
            successors.append((southWestState,1))

        if action == 'southeast':
            successors.append((southEastState,1))
...
def getMapGrid():
	grid = cv2.imread('maps/cross_walk.png')
	grid = cv2.imread('maps/cross_walk_multiple.png')
	grid = cv2.resize(grid, (10,10))
	grid = np.argmax(grid, 2).tolist()
...
def parseOptions():
    ...
    optParser.add_option('-d', '--discount',action='store',
                         type='float',dest='discount',default=0.99,
                         help='Discount on future (default %default)')
    optParser.add_option('-r', '--livingReward',action='store',
                         type='float',dest='livingReward',default=-0.3,
    ...
    optParser.add_option('-g', '--grid',action='store',
                         metavar="G", type='string',dest='grid',default="MapGrid",
                         help='Grid to use (case sensitive; options are BookGrid, BridgeGrid, CliffGrid, MazeGrid, default %default)' )
    optParser.add_option('-w', '--windowSize', metavar="X", type='int',dest='gridSize',default=50,
